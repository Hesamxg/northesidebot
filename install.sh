...
function self_update_script() {
    ...
    local BIN_LINK="/usr/local/bin/northeside"
    local URL="https://raw.githubusercontent.com/Hesamxg/northesidebot/main/install.sh"
...
function show_logo() {
    ...
    echo -e "\033[1;36m+-------------------+---------------------------------------------------+\033[0m"
    echo -e "\033[1;36m| Telegram Channel  |\033[0m \033[34mhttps://t.me/NorThSiDe_Team1\033[0m"
    echo -e "\033[1;36m+-------------------+---------------------------------------------------+\033[0m"
    echo -e "\033[1;36m| Telegram Group    |\033[0m \033[34mhttps://t.me/NorThSiDe_Team1\033[0m"
    echo -e "\033[1;36m+-------------------+---------------------------------------------------+\033[0m"
    echo ""
...
function show_menu() {
    show_logo
    echo -e "\033[1;36m1)\033[0m Install NorThe SiDe Bot"
    echo -e "\033[1;36m2)\033[0m Update NorThe SiDe Bot"
    echo -e "\033[1;36m3)\033[0m Remove NorThe SiDe Bot"
...
# Install Function for NorThe SiDe Pro
function install_bot() {
    echo -e "\e[32mInstalling NorThe SiDe Pro script ... \033[0m\n"
    ...
    ZIP_URL="https://github.com/Hesamxg/northesidebot/archive/refs/heads/main.zip"
    echo -e "\033[33mDownloading NorThe SiDe Pro from Main Branch...\033[0m"
    ...
    echo -e "\n\033[33mNorThe SiDe Pro config and script have been installed successfully.\033[0m"
    wait
...
EOF
     ...
     "https://api.telegram.org/bot${YOUR_BOT_TOKEN}/setWebhook" || {
            ...
            MESSAGE="✅ The NorThe SiDe Pro bot is installed! for start the bot send /start command."
    ...
    ln -sf /root/install.sh /usr/local/bin/northeside
    # Trigger self-update to ensure next run uses latest
    self_update_script
}
# function install_bot_with_marzban() {
#     # Display warning and confirmation
#     echo -e "\033[41m[IMPORTANT WARNING]\033[0m \033[1;33mMarzban panel is detected on your server. Please make sure to backup the Marzban database before installing NorThe SiDe Bot.\033[0m"
#     read -p "Are you sure you want to install NorThe SiDe Bot alongside Marzban? (y/n): " confirm
...
#         echo -e "\e[91mError: Your database is $DB_TYPE. To install NorThe SiDe Bot, you must use MySQL.\033[0m"
...
#     clear
#     echo -e "\e[33mConfiguring NorThe SiDe Bot database credentials...\033[0m"
...
#     chmod +x /root/install.sh
#     ln -vs /root/install.sh /usr/local/bin/northeside
# }
# Update Function for NorThe SiDe Pro
function update_bot() {
    echo "Updating NorThe SiDe Bot..."
    ...
    BOT_DIR="/var/www/html/mirzaprobotconfig"
    if [ ! -d "$BOT_DIR" ]; then
        echo -e "\e[91mError: NorThe SiDe Bot is not installed. Please install it first.\033[0m"
        exit 1
    fi
...
EOF
    ...
    # Cleanup
    rm -rf "$TEMP_DIR"
    echo -e "\n\e[92mNorThe SiDe Bot updated to latest version successfully!\033[0m"
    # Ensure /root/install.sh is executable and linked to mirza
    if [ -f "/root/install.sh" ]; then
        sudo chmod +x /root/install.sh
        sudo ln -sf /root/install.sh /usr/local/bin/northeside
...
# Delete Function for NorThe SiDe Pro
function remove_bot() {
    echo -e "\e[33mStarting NorThe SiDe Bot removal process...\033[0m"
    LOG_FILE="/var/log/remove_bot.log"
    echo "Log file: $LOG_FILE" > "$LOG_FILE"
    # Check if NorThe SiDe Bot is installed
    BOT_DIR="/var/www/html/mirzaprobotconfig"
    if [ ! -d "$BOT_DIR" ]; then
        echo -e "\e[31m[ERROR]\033[0m NorThe SiDe Bot is not installed (/var/www/html/mirzaprobotconfig not found)." | tee -a "$LOG_FILE"
        echo -e "\e[33mNothing to remove. Exiting...\033[0m" | tee -a "$LOG_FILE"
        sleep 2
        exit 1
    fi
    # User Confirmation
    read -p "Are you sure you want to remove NorThe SiDe Bot and its dependencies? (y/n): " choice
    if [[ "$choice" != "y" ]]; then
    ...
    echo "Removing NorThe SiDe Bot..." | tee -a "$LOG_FILE"
    ...
    sudo ufw reload
    echo -e "\e[92mNorThe SiDe Bot, MySQL, and their dependencies have been completely removed.\033[0m" | tee -a "$LOG_FILE"
}
# function remove_bot_with_marzban() {
#     echo -e "\e[33mRemoving NorThe SiDe Bot alongside Marzban...\033[0m" | tee -a "$LOG_FILE"
...
#     }
#     sudo ufw reload
#     echo -e "\e[92mNorThe SiDe Bot has been removed alongside Marzban. SSL certificates remain intact.\033[0m" | tee -a "$LOG_FILE"
# }
...
#     BOT_DIR="/var/www/html/mirzabotconfig"
#     if [ ! -d "$BOT_DIR" ]; then
#         echo -e "\033[31m[ERROR]\033[0m NorThe SiDe Bot is not installed ($BOT_DIR not found)."
...
function migrate_to_pro() {
    ...
    # Download Pro Source
    echo -e "\033[33mDownloading NorThe SiDe Pro Source...\033[0m"
    ZIP_URL="https://github.com/Hesamxg/northesidebot/archive/refs/heads/main.zip"
...
EOF
    ...
    cp /root/install.sh /usr/local/bin/northeside
    chmod +x /usr/local/bin/northeside
...
process_arguments() {
    case "$1" in
        ...
        *)
            ...
            if [ -n "$1" ]; then
                echo -e "\e[33mNote: NorThe SiDe Pro only uses the latest version from GitHub Main branch.\033[0m"
            fi
            show_menu
            ;;
    esac
